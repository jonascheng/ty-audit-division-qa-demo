...
class Embeddings(metaclass=abc.ABCMeta):
    ...
    def run(self):
        ...
        # output the first document for debugging purpose
        logger.debug(f'First document: {documents[0]}')

        # cut documents to {PERCENTAGE_OF_DOCUMENTS_TO_BE_PROCESSED}%
        if PERCENTAGE_OF_DOCUMENTS_TO_BE_PROCESSED < 100:
            logger.info(
                f'Cutting documents to {PERCENTAGE_OF_DOCUMENTS_TO_BE_PROCESSED}%')
            documents = documents[:int(
                len(documents)*PERCENTAGE_OF_DOCUMENTS_TO_BE_PROCESSED/100)]

        # replace CJK space with normal space
        logger.info('Replacing CJK space with normal space')
        documents = self._remove_cjk_space(documents)
